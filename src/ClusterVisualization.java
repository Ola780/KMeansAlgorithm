...
class ClusterVisualization extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        double minX = dataPoints.stream().map(e -> e.getFeatures()[0]).min(Double::compareTo).orElse(0.0);
        double maxX = dataPoints.stream().map(e -> e.getFeatures()[0]).max(Double::compareTo).orElse(1.0);
        double minY = dataPoints.stream().map(e -> e.getFeatures()[1]).min(Double::compareTo).orElse(0.0);
        double maxY = dataPoints.stream().map(e -> e.getFeatures()[1]).max(Double::compareTo).orElse(1.0);
